...
app.get('/comments/:episode', async (req, res) => {
  try {
    const comments = await Comment.find({ episode: req.params.episode });
    res.json(comments);
  } catch (err) {
...
app.post('/comments', async (req, res) => {
  const { episode, name, comment } = req.body;

  if (!episode || !name || !comment) {
    return res.status(400).json({ error: 'All fields are required' });
  }

  try {
    const newComment = new Comment({ episode, name, comment });
